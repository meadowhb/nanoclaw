...
describe('run-migrations', () => {
  ...
  const localTsxBin = path.resolve('node_modules/.bin/tsx');
  const npxCommand = process.platform === 'win32' ? 'npx.cmd' : 'npx';
  const tsxRunner = (() => {
    if (fs.existsSync(localTsxBin)) {
      return { command: localTsxBin, argsPrefix: [] as string[] };
    }
    if (process.platform !== 'win32') {
      try {
        const cachedTsxBin = execFileSync(
          npxCommand,
          ['--yes', '--package=tsx', 'which', 'tsx'],
          {
            encoding: 'utf-8',
        ...
        if (cachedTsxBin) {
          return { command: cachedTsxBin, argsPrefix: [] as string[] };
        }
      } catch {
        // Fall through to npx invocation below.
      }
    }
  ...
  function runMigrations(
    ...
    try {
      const stdout = execFileSync(
        tsxRunner.command,
        [...tsxRunner.argsPrefix, scriptPath, from, to, newCoreDir],
        {
          cwd: tmpDir,
          encoding: 'utf-8',
          stdio: 'pipe',
